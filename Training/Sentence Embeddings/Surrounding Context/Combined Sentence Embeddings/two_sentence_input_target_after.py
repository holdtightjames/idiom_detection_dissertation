...
# The sentences after the idiom and the idiom are inputted into BERT 
# =============================================================================
...
sentence_after_and_target_dict = {}
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')

sem_eval_dataset = pd.read_csv('sem_eval_train_one_shot.csv')

# loading in the sentences before and the target
target_sentences = sem_eval_dataset.query('Label==0')['Target']
sentences_after = sem_eval_dataset.query('Label==0')['Next']


# Combines the sentences after and the target sentences to create a 2D array of them both
sentence_after_and_target = zip(target_sentences,sentences_after)
...
i = 0
for target,sentence_after in sentence_after_and_target:
    i = i + 1
    sentence_vector = sentence_embedding_two_inputs(target,sentence_after)
    sentence_after_and_target_dict[i] = sentence_vector


with open('two_sentence_input_embeddings_after_target.csv','wt', encoding='UTF8') as out_file:
    headersList =['Sentence_num','Sentence Vector']
    tsv_writer = csv.writer(out_file)
    tsv_writer.writerow(headersList)
    for sent_id,vector in sentence_after_and_target_dict.items():
